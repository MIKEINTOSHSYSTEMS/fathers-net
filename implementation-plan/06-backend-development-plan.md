...
> **Migration numbering (this plan vs `05`).** Database schema detail is owned by `05-database-implementation-plan.md` §4.2, which is authoritative for migration IDs and grouping. The per-phase "Database changes" bullets below reference `05` §4.2 migration IDs; they do **not** define a competing `000N` sequence. Tables listed in a phase that are **not** in the `05` §4.2 catalog (e.g., `shared_journey_links`, `data_export_jobs`, `whatsapp_templates`, `support_tickets`) are engineering additions that must be added to `05` §4.2 (with schema approval + a `decision-log.md` entry) before that phase lands. `reminder_templates` / `reminder_instances` / `reminder_dispatches` were added to `05` §4.2 as migration **018** on 2026-08-07 (decision-log **D-10**, WP-021). The auth tables proposed in Phase B are subject to the **closed** auth-state storage decision (`05` §4.3 — **DECIDED 2026-08-06, Option A: Redis-only**; no auth tables in Phase 2).
...
**Database changes.** `appointments` (§13.3.15) lands in `05` §4.2 migration 010 (`budget-and-appointments`), `notifications` (§13.3.25) in migration 014. `reminder_templates`, `reminder_instances`, `reminder_dispatches` were added to `05` §4.2 as migration **018** on 2026-08-07 (decision-log **D-10**, WP-021); `reminder_events`, `milestones`, `support_actions` remain additions beyond the `05` catalog (see §4 numbering note). Indexes on `appointments.scheduled_at`, `notifications.user_id`.
