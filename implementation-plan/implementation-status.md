...
| WP-016 | Auth service (initial) | S | Auth-state storage decision **closed 2026-08-06 (Option A — Redis-only**, `05` §4.3, `decision-log.md` D-09): OTP + revoked/refresh state in Redis via provider-agnostic adapter + test-double (M-08), hashed values only; no auth tables/migrations; full rotation + reuse-detection = Phase 3 (WP-025). **Awaits WP-016 authorization.** | Backend |
