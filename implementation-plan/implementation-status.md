...
| WP-021 | Reminder engine | IP | Implemented locally 2026-08-07 (evidence: `wp-021-progress-report.md`): `services/reminders` engine + memory/Postgres adapters + internal API + job factory; scheduler registers `createRemindersJobs` (WP-024b hook); contract `reminders-internal.yaml` contract-lint gated; migration `018` committed + pushed (`639c469`). Local gates all green — reminders 91 tests + 92.57% lines (8 Postgres integration tests env-gated), repo typecheck 20/20 / lint 14/14 / test 20/20, contract:lint 5/5 (0 warnings). **Uncommitted working tree — awaits separate commit authorization.** | Backend |
