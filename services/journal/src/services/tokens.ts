...
/**
 * Stateless access-token verifier for the journal service (WP-016 identity
 * context, AR-008). Validates the HS256 signature, issuer, audience, expiry,
 * and `typ: access` claim using the shared auth secret — the same contract the
 * gateway, users and content services implement. The `sub` claim is the caller
 * identity for every ownership-scoped journal handler (FR-126); the caller's
