 ...
 * the best-effort `reminder.due` publisher. Run-id binding is the FR-163
 ...
 *
 * When the factory builds its own service the store pool and bus are
...
function buildOwnedService(config: RemindersConfig, logger: Logger | undefined): ReminderService {
  ...
  return createReminderService({ store, dispatcher, bus, logger, config });
}
