...
/**
 * Stateless access-token verifier for the reminders service (WP-016 identity
 * context). Validates the HS256 signature, issuer, audience, expiry, and
 * `typ: access` claim using the shared auth secret — the same contract the
 * gateway and the content/users services implement (AR-008). The `sub` claim
 * is the caller's identity for all ownership checks on the internal contract.
 */
