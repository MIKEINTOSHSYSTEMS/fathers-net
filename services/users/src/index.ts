...
import { buildUsersApp } from './app';

const config = loadUsersConfig(process.env);
...
async function main(): Promise<void> {
  const app = await buildUsersApp({ config, logger });
  ...
  try {
    ...
    logger.info('app.started', 'users service listening', {
      host: config.FN_HOST,
      port: config.FN_PORT,
      version: config.FN_VERSION,
    });
  } catch (err) {
    logger.error('app.fatal', 'failed to start users service', { err: String(err) });
    process.exit(1);
  }
}
